...
#include <map>
#include <vector>
#include <sys/epoll.h> // For epoll on Linux/Ubuntu

// Forward declarations so you don't get circular dependency issues
class User;
class Channel;
...
class Server {
    ...
    public:
        ...
        //Command channel
        void join(std::string nameChannel, std::string key, const User &client);
        void join(std::string channel, const User &client);
};
